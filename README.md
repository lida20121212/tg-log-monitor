...
默认排除 `"response": {"code":40102`、`"response": {"code":40103`、`"response": {"code":40205`、`"error": "Verification failed", "response": {"code":1,"msg":"Verification failed"`、`This device is already registered...mobile phone number ending in...`、`Field validation for ... failed on the ... tag`，以及 `paymentchannel/`、`payment_channel.go`、`cash_payout.go`、`luckyinpay`、`empty callback payload`、支付/代付回调这类已知三方通道错误，不发送到 TG 群。
